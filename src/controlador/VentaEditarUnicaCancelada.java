...
public class VentaEditarUnicaCancelada extends Controlador implements Initializable {
    ...
    private  JFXTreeTableView<VentaMostrador> cargarTabla(JFXTreeTableView<VentaMostrador> fxTreeTableView) {
        ...
        JFXTreeTableColumn<VentaMostrador, ImageView> columnImagen = new JFXTreeTableColumn<>("Imágen");
        ...
        columnCantidad.setOnEditCommit((TreeTableColumn.CellEditEvent<VentaMostrador, String> t) -> {
                    ...
                    if(cantidadNueva<cantidadActual) {
                        ...
                        VentaMostrador vM = new VentaMostrador(idProducto, cantidadNueva,producto,costo, idVentaProductosDetalle);
                        ...
                        VentaMostrador vM = new VentaMostrador(idProducto, cantidadActual,producto,costo, idVentaProductosDetalle);
                        listasVentasMostrador.get(nVentaSelect).remove(index);
                        listasVentasMostrador.get(nVentaSelect).add( vM);
                        t.getTreeTableView().getSelectionModel().select(listasVentasMostrador.get(nVentaSelect).size()-1);
                    }
    ...
    public void init() {
        ...
        try {
            agregarProducto(Configuraciones.idVentaProductos);
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private void agregarProducto(int idVentaProductos) throws IOException {




        Tab tab = new Tab();
        tab.setText(idVentaProductos+"");
        TablasVentas.add(cargarTabla(new JFXTreeTableView<>() ));
        tab.setContent(TablasVentas.get(TablasVentas.size()-1));
        Tabs.getTabs().add(tab);
        Tabs.getSelectionModel().select(Tabs.getTabs().size()-1);
        ...
        Map<String,Object> paramsJSON = new LinkedHashMap<>();
        paramsJSON.put("Actividad", "Venta Productos Cancelada: Listar detalles");
        paramsJSON.put("idVentaProductos", idVentaProductos);
